...
public class SymmetricHashJoinTest {
  ...
  public void testSymmetricHashJoin() throws DbException {
    ...
    Operator join = new SymmetricHashJoin(left, right, new int[] { 1, 0, 2 }, new int[] { 2, 1, 0 }, new int[] { 0 },
        new int[] { 0 });
    join.open(TestEnvVars.get());
    assertEquals(Schema.merge(JoinTestUtils.leftSchema.getSubSchema(new int[] { 0 }), JoinTestUtils.rightSchema
        .getSubSchema(new int[] { 0 })), join.getSchema());
    long count = 0;
    while (!join.eos()) {
  ...
  public void testIncompatibleJoinKeys() throws DbException {
    TupleSource left = new TupleSource(JoinTestUtils.leftInput);
    TupleSource right = new TupleSource(JoinTestUtils.rightInput);
    Operator join = new SymmetricHashJoin(left, right, new int[] { 0 }, new int[] { 0 }, new int[] { 0 }, new int[] {
        0 });
    join.open(TestEnvVars.get());
  }
}
