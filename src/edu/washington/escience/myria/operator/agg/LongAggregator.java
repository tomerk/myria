...
public final class LongAggregator extends PrimitiveAggregator {
  ...
  protected LongAggregator(final String inputName, final int column, final AggregationOp aggOp, final int[] stateCols) {
  ...
  public void addRow(final ReadableTable from, final int fromRow, final MutableTupleBuffer to, final int toRow) {
    ...
    switch (aggOp) {
      ...
      case MIN:
        ...
        break;
      case SUM:
        toCol.replaceLong(LongMath.checkedAdd(fromCol.getLong(fromRow), toCol.getLong(toRow)), toRow);
        break;
      case SUM_SQUARED:
        toCol.replaceLong(LongMath.checkedAdd(LongMath.checkedMultiply(fromCol.getLong(fromRow), fromCol.getLong(
            fromRow)), toCol.getLong(toRow)), toRow);
        break;
      default:
